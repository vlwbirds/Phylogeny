...
```{r}
plotTree(pruned_tree, type = "fan", ftype = "i")
```
...
fit <- fastAnc(pruned_tree, alarm, vars = TRUE, CI = TRUE)
fit
```
...
```{r}
obj2 <- contMap(pruned_tree, alarm, plot = FALSE)
plot( setMap(obj, invert = TRUE), legend = 0.7*max(nodeHeights()),
     fsize = c(0.7, 0.9))
```
